...
import { createOrder, getAllProductsForSelection, isAxiosError } from '../services/storeApi'
...
async function loadProducts(): Promise<void> {
  ...
  try {
    ...
    apiError.value = 'Unable to load the product catalogue.'
  } finally {
  ...
  try {
    ...
    if (isAxiosError(error)) {
      ...
      if (data?.error === 'validation_failed' && data?.errors) {
        ...
        apiError.value = 'Validation failed. Review the highlighted fields.'
        return
      }

      if (data?.error === 'insufficient_stock') {
        apiError.value = `Insufficient stock for product ${data.product_id ?? 'selected'} requested.`
        return
      }

      apiError.value = data?.message ?? 'The order could not be created.'
      return
    }
